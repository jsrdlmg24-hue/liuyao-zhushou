...
import { COIN_THROW_OPTIONS, DEFAULT_NUMBERS, LINE_LABELS, isValidYaoNumber } from './ui/coinOptions.js'
import { renderPlateViewModelHtml, escapeHtml } from './ui/plateRenderer.js'
...
let state = { records: [], settings: { token: '', gistId: '', lastSync: '' }, currentRecordId: '', search: '' }
...
function makeId() { const date = todayId(); const count = state.records.filter(r => r.id.startsWith(`LY-${date}`)).length + 1; return `LY-${date}-${String(count).padStart(3, '0')}` }
...
function saveLocal() { localStorage.setItem(STORE_KEY, JSON.stringify(state.records)); localStorage.setItem(SETTINGS_KEY, JSON.stringify(state.settings)); localStorage.setItem(CURRENT_KEY, state.currentRecordId || ''); updateHeader() }
function currentRecord() { return state.records.find(r => r.id === state.currentRecordId) }
function ensureRecord() { if (currentRecord()) return currentRecord(); if (!state.records.length) return null; state.currentRecordId = state.records[0].id; saveLocal(); return currentRecord() }
function ensurePlateInput(record) {
  if (!record.plateInput) record.plateInput = { question: record.question || '', castTime: localDateTimeValue(), numbers: [...DEFAULT_NUMBERS], collapsed: false }
  if (!Array.isArray(record.plateInput.numbers) || record.plateInput.numbers.length !== 6) record.plateInput.numbers = [...DEFAULT_NUMBERS]
  record.plateInput.numbers = record.plateInput.numbers.map(n => isValidYaoNumber(n) ? Number(n) : 8)
...
function newRecord(title = '', question = '') {
  const id = makeId()
  const r = { id, title: title.trim() || `读盘笔记 ${id}`, question: question.trim(), createdAt: nowIso(), updatedAt: nowIso(), tags: [], plateInput: { question: question.trim(), castTime: localDateTimeValue(), numbers: [...DEFAULT_NUMBERS], collapsed: false }, steps: {}, final: {} }
  STEPS.forEach(s => r.steps[s.id] = { conclusion: '', evidence: '', doubt: '', note: '', fills: Array(s.fill.length).fill('') })
  state.records.unshift(r); state.currentRecordId = id; saveLocal(); return r
}
...
function buildCurrentPlateVm() { const record = currentRecord(); if (!record) return null; const input = ensurePlateInput(record); try { return buildPlateViewModel({ question: input.question || record.question || '', castTime: input.castTime, numbers: input.numbers }) } catch (err) { console.error('buildPlateViewModel failed:', err); return null } }
function setActiveTab() { const hash = location.hash || '#home'; qsa('.bottom-nav a').forEach(a => { const tab = a.dataset.tab; a.classList.toggle('active', hash.startsWith(`#${tab}`) || (tab === 'home' && hash.startsWith('#step-')) || (tab === 'knowledge' && hash.startsWith('#knowledge-'))) }) }
function go(hash) { location.hash = hash }

function render() {
  setActiveTab(); updateHeader(); renderPlateDock()
...
function renderHome() {
  const app = qs('#app'); const r = ensureRecord()
  app.innerHTML = `<section class="card hero"><h1>15 步读盘流程</h1><p>先排盘，再按步骤写证据。每步笔记会自动保存，最后汇总成断语。</p><p class="small">当前笔记：${r ? `${escapeHtml(r.id)} · ${escapeHtml(r.title)}` : '未选择'}</p><div class="button-row"><a href="#plate" class="primary-btn">进入排盘</a><a href="#records" class="secondary-btn">新建 / 切换笔记</a><a href="#summary" class="secondary-btn">自动汇总</a></div></section><section class="card compact"><div class="kicker">资料权重</div><p><b>主裁判：</b>纳甲六爻 / 文王卦 / 火珠林。先看六亲、世应、用神、月日、动变、生克冲合、旬空月破。</p><p><b>辅助：</b>古法规则、朱辰彬流程优先级、王虎应现代分类、六神爻位卦象解释。</p></section><section class="step-list">${STEPS.map(s => `<a class="step-card" href="#step-${s.id}"><div class="step-no">${s.id}</div><div><div class="step-title">${escapeHtml(s.title)}</div><div class="step-desc">${escapeHtml(s.desc)}</div></div><div class="chev">›</div></a>`).join('')}</section><div class="footer-space"></div>`
}

function renderPlatePage() {
  const app = qs('#app'); const record = ensureRecord()
  if (!record) { app.innerHTML = recordNeededHtml(); return }
  const input = ensurePlateInput(record); const vm = buildCurrentPlateVm()
  app.innerHTML = `<section class="card"><div class="kicker">排盘输入</div><h1>六爻排盘</h1><p class="small">铜钱标准：字面/有字/年号面 = 阴 = 2；背面/无字/无年号面 = 阳 = 3。输入顺序固定：初爻 → 二爻 → 三爻 → 四爻 → 五爻 → 上爻。</p><label>占问</label><textarea id="plateQuestion" placeholder="例如：这次面试能不能通过？">${escapeHtml(input.question || '')}</textarea><label>起卦时间</label><input id="plateCastTime" type="datetime-local" value="${escapeHtml(input.castTime)}" /><h2>六次投掷结果</h2><div class="yao-input-grid">${LINE_LABELS.map((label, idx) => `<div class="yao-input-row"><div class="line-label">${label}</div><select data-plate-yao-index="${idx}">${COIN_THROW_OPTIONS.map(opt => `<option value="${opt.value}" ${input.numbers[idx] === opt.value ? 'selected' : ''}>${opt.text}</option>`).join('')}</select></div>`).join('')}</div><div class="button-row"><button class="primary-btn" id="buildPlateBtn">排盘 / 刷新盘面</button><button class="secondary-btn" id="resetPlateBtn">重置</button></div></section><section class="card"><h2>完整盘面</h2><p class="small">显示风格按传统排盘：表格视觉从上爻往下；输入和核心计算仍按初爻到上爻。天干地支颜色代表五行。</p>${renderFullPlateHtml(vm)}</section>`
  bindPlatePage()
}
function bindPlatePage() {
  const record = currentRecord(); if (!record) return; const input = ensurePlateInput(record)
...
function renderPlateDock() {
  const dock = qs('#plateDock'); if (!dock) return
  const record = currentRecord(); if (!record) { dock.innerHTML = '<div class="plate-empty">未选择读盘笔记。请先新建或切换笔记。</div>'; return }
  const input = ensurePlateInput(record); const vm = buildCurrentPlateVm()
  if (!vm) { dock.innerHTML = '<div class="plate-empty">排盘数据有误，请检查投掷结果和起卦时间。</div>'; return }
  dock.innerHTML = `<div class="plate-card ${input.collapsed ? 'collapsed' : ''}"><div class="plate-head"><div><div class="plate-title">${escapeHtml(vm.originalHexagram.name)} → ${escapeHtml(vm.changedHexagram.name)}</div><div class="plate-sub">${escapeHtml(vm.header.castTimeText)}｜${escapeHtml(vm.header.baziText)}｜动爻：${vm.movingPositions.length ? vm.movingPositions.join('、') : '无'}</div></div><button class="ghost-btn small-btn" id="togglePlateDock">${input.collapsed ? '展开' : '收起'}</button></div>${input.collapsed ? '' : renderFullPlateHtml(vm, { compact: true })}</div>`
  qs('#togglePlateDock')?.addEventListener('click', () => { input.collapsed = !input.collapsed; touchRecord(record); renderPlateDock() })
}
function renderFullPlateHtml(vm, options = {}) { return renderPlateViewModelHtml(vm, options) }

function renderStep(stepId) {
  const app = qs('#app'); const step = STEPS.find(s => s.id === stepId) || STEPS[0]; const r = ensureRecord()
  ...
  app.innerHTML = `<section class="card"><div class="kicker">第 ${step.id} 步</div><h1>${escapeHtml(step.title)}</h1><p>${escapeHtml(step.desc)}</p><div class="callout ok"><b>本步产出：</b>${escapeHtml(step.output)}</div><h3>本步知识点</h3><div class="tag-row">${step.knowledge.map(kid => { const k = KNOWLEDGE.find(x => x.id === kid); return `<a class="knowledge-chip" href="#knowledge-${kid}">${escapeHtml(k?.title || kid)}</a>` }).join('')}</div></section><section class="card"><h2>本步填空</h2>${step.fill.map((prompt, i) => `<div class="fill-card"><label>${escapeHtml(prompt)}</label><input type="text" data-step-field="fill" data-index="${i}" value="${escapeHtml(data.fills?.[i] || '')}" placeholder="点这里填写" /></div>`).join('')}</section><section class="card"><h2>本步笔记</h2>${stepField('conclusion', '本步结论', data.conclusion)}${stepText('evidence', '盘面证据', data.evidence)}${stepText('doubt', '疑点 / 待复核', data.doubt)}${stepText('note', '自由笔记', data.note, 'note-area')}</section><section class="card compact"><div class="button-row">${step.id > 1 ? `<a class="secondary-btn" href="#step-${step.id - 1}">上一步</a>` : ''}${step.id < STEPS.length ? `<a class="primary-btn" href="#step-${step.id + 1}">下一步</a>` : '<a class="primary-btn" href="#summary">去汇总</a>'}<a class="ghost-btn" href="#home">回流程</a></div></section>`
...
function renderRecords() {
  const app = qs('#app')
  app.innerHTML = `<section class="card"><h1>读盘笔记</h1><p>每一次占事是一条独立笔记，含自己的问题、起卦时间、六爻数和步骤笔记。</p><div class="form-block"><label>新笔记名称</label><input id="newTitle" type="text" placeholder="例如：找钥匙 / 面试结果 / 求财" /><label>问题原文</label><textarea id="newQuestion" placeholder="例如：今天丢的钥匙能不能找回来？"></textarea><button class="primary-btn full" id="createRecordBtn">新建读盘笔记</button></div></section><section class="card"><h2>已有笔记</h2><div class="grid">${state.records.length ? state.records.map(recordCardHtml).join('') : '<p class="small">还没有笔记。</p>'}</div></section>`
  qs('#createRecordBtn').addEventListener('click', () => { newRecord(qs('#newTitle').value, qs('#newQuestion').value); go('#plate') })
...
function init() { loadLocal(); window.addEventListener('hashchange', render); if ('serviceWorker' in navigator && import.meta.env.PROD) navigator.serviceWorker.register('./sw.js').catch(console.warn); render() }
